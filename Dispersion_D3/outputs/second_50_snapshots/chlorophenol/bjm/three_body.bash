...
for i in $(find ../../${ROOT}/${LIGNAME}_next50/ligand/ -name "*.xyz")
do 
  echo $i
  ...
  SNAP=$(echo $i | cut -d '/' -f 6 | cut -d '_' -f 2 | cut -d '.' -f 1)
  echo $SNAP
...
for i in $(find ../../${ROOT}/${LIGNAME}_next50/host/ -name "*.xyz")
do 
  echo $i
  $SOURCE/dftd3 $i $FLAGS > $TARGET/disp.temp
  #SNAP=$(echo $i | cut -d "_" -f 5 | cut -d "." -f 1)
...
for i in $(find ../../${ROOT}/${LIGNAME}_next50/complex/ -name "*.xyz")
do 
  echo $i
  $SOURCE/dftd3 $i $FLAGS > $TARGET/disp.temp
  #SNAP=$(echo $i | cut -d "_" -f 5 | cut -d "." -f 1)
